...
},Popover.prototype.setContent=function(){var $tip=this.tip(),title=this.getTitle(),content=this.getContent();$tip.find(".popover-title")[this.options.html?"html":"text"](title),$tip.find(".popover-content").children().detach().end()[this.options.html?"string"==typeof content?"html":"append":"text"](content),$tip.removeClass("fade top bottom left right in"),$tip.find(".popover-title").html()||$tip.find(".popover-title").hide()},Popover.prototype.hasContent=function(){return this.getTitle()||this.getContent()},Popover.prototype.getContent=function(){var $e=this.$element,o=this.options;return $e.attr("data-content")||("function"==typeof o.content?o.content.call($e[0]):o.content)},Popover.prototype.arrow=function(){return this.$arrow=this.$arrow||this.tip().find(".arrow")},Popover.prototype.tip=function(){return this.$tip||(this.$tip=$(this.options.template)),this.$tip};var old=$.fn.popover;$.fn.popover=Plugin,$.fn.popover.Constructor=Popover,$.fn.popover.noConflict=function(){return $.fn.popover=old,this}}(jQuery),+function($){"use strict";function ScrollSpy(element,options){var process=$.proxy(this.process,this);this.$body=$("body"),this.$scrollElement=$($(element).is("body")?window:element),this.options=$.extend({},ScrollSpy.DEFAULTS,options),this.selector=(this.options.target||"")+" .nav li > a",this.offsets=[],this.targets=[],this.activeTarget=null,this.scrollHeight=0,this.$scrollElement.on("scroll.bs.scrollspy",process),this.refresh(),this.process()}function Plugin(option){return this.each(function(){var $this=$(this),data=$this.data("bs.scrollspy"),options="object"==typeof option&&option;data||$this.data("bs.scrollspy",data=new ScrollSpy(this,options)),"string"==typeof option&&data[option]()})}ScrollSpy.VERSION="3.3.0",ScrollSpy.DEFAULTS={offset:10},ScrollSpy.prototype.getScrollHeight=function(){return this.$scrollElement[0].scrollHeight||Math.max(this.$body[0].scrollHeight,document.documentElement.scrollHeight)},ScrollSpy.prototype.refresh=function(){var offsetMethod="offset",offsetBase=0;$.isWindow(this.$scrollElement[0])||(offsetMethod="position",offsetBase=this.$scrollElement.scrollTop()),this.offsets=[],this.targets=[],this.scrollHeight=this.getScrollHeight();var self=this;this.$body.find(this.selector).map(function(){var $el=$(this),href=$el.data("target")||$el.attr("href"),$href=/^#./.test(href)&&$(href);return $href&&$href.length&&$href.is(":visible")&&[[$href[offsetMethod]().top+offsetBase,href]]||null}).sort(function(a,b){return a[0]-b[0]}).each(function(){self.offsets.push(this[0]),self.targets.push(this[1])})},ScrollSpy.prototype.process=function(){var i,scrollTop=this.$scrollElement.scrollTop()+this.options.offset,scrollHeight=this.getScrollHeight(),maxScroll=this.options.offset+scrollHeight-this.$scrollElement.height(),offsets=this.offsets,targets=this.targets,activeTarget=this.activeTarget;if(this.scrollHeight!=scrollHeight&&this.refresh(),scrollTop>=maxScroll)return activeTarget!=(i=targets[targets.length-1])&&this.activate(i);if(activeTarget&&scrollTop<offsets[0])return this.activeTarget=null,this.clear();for(i=offsets.length;i--;)activeTarget!=targets[i]&&scrollTop>=offsets[i]&&(!offsets[i+1]||scrollTop<=offsets[i+1])&&this.activate(targets[i])},ScrollSpy.prototype.activate=function(target){this.activeTarget=target,this.clear();var selector=this.selector+'[data-target="'+target+'"],'+this.selector+'[href="'+target+'"]',active=$(selector).parents("li").addClass("active");active.parent(".dropdown-menu").length&&(active=active.closest("li.dropdown").addClass("active")),active.trigger("activate.bs.scrollspy")},ScrollSpy.prototype.clear=function(){$(this.selector).parentsUntil(this.options.target,".active").removeClass("active")};var old=$.fn.scrollspy;$.fn.scrollspy=Plugin,$.fn.scrollspy.Constructor=ScrollSpy,$.fn.scrollspy.noConflict=function(){return $.fn.scrollspy=old,this},$(window).on("load.bs.scrollspy.data-api",function(){$('[data-spy="scroll"]').each(function(){var $spy=$(this);Plugin.call($spy,$spy.data())})})}(jQuery),+function($){"use strict";function Plugin(option){return this.each(function(){var $this=$(this),data=$this.data("bs.tab");data||$this.data("bs.tab",data=new Tab(this)),"string"==typeof option&&data[option]()})}var Tab=function(element){this.element=$(element)};Tab.VERSION="3.3.0",Tab.TRANSITION_DURATION=150,Tab.prototype.show=function(){var $this=this.element,$ul=$this.closest("ul:not(.dropdown-menu)"),selector=$this.data("target");if(selector||(selector=$this.attr("href"),selector=selector&&selector.replace(/.*(?=#[^\s]*$)/,"")),!$this.parent("li").hasClass("active")){var $previous=$ul.find(".active:last a"),hideEvent=$.Event("hide.bs.tab",{relatedTarget:$this[0]}),showEvent=$.Event("show.bs.tab",{relatedTarget:$previous[0]});if($previous.trigger(hideEvent),$this.trigger(showEvent),!showEvent.isDefaultPrevented()&&!hideEvent.isDefaultPrevented()){var $target=$(selector);this.activate($this.closest("li"),$ul),this.activate($target,$target.parent(),function(){$previous.trigger({type:"hidden.bs.tab",relatedTarget:$this[0]}),$this.trigger({type:"shown.bs.tab",relatedTarget:$previous[0]})})}}},Tab.prototype.activate=function(element,container,callback){function next(){$active.removeClass("active").find("> .dropdown-menu > .active").removeClass("active").end().find('[data-toggle="tab"]').attr("aria-expanded",!1),element.addClass("active").find('[data-toggle="tab"]').attr("aria-expanded",!0),transition?(element[0].offsetWidth,element.addClass("in")):element.removeClass("fade"),element.parent(".dropdown-menu")&&element.closest("li.dropdown").addClass("active").end().find('[data-toggle="tab"]').attr("aria-expanded",!0),callback&&callback()}var $active=container.find("> .active"),transition=callback&&$.support.transition&&($active.length&&$active.hasClass("fade")||!!container.find("> .fade").length);$active.length&&transition?$active.one("bsTransitionEnd",next).emulateTransitionEnd(Tab.TRANSITION_DURATION):next(),$active.removeClass("in")};var old=$.fn.tab;$.fn.tab=Plugin,$.fn.tab.Constructor=Tab,$.fn.tab.noConflict=function(){return $.fn.tab=old,this};var clickHandler=function(e){e.preventDefault(),Plugin.call($(this),"show")};$(document).on("click.bs.tab.data-api",'[data-toggle="tab"]',clickHandler).on("click.bs.tab.data-api",'[data-toggle="pill"]',clickHandler)}(jQuery),+function($){"use strict";function Plugin(option){return this.each(function(){var $this=$(this),data=$this.data("bs.affix"),options="object"==typeof option&&option;data||$this.data("bs.affix",data=new Affix(this,options)),"string"==typeof option&&data[option]()})}var Affix=function(element,options){this.options=$.extend({},Affix.DEFAULTS,options),this.$target=$(this.options.target).on("scroll.bs.affix.data-api",$.proxy(this.checkPosition,this)).on("click.bs.affix.data-api",$.proxy(this.checkPositionWithEventLoop,this)),this.$element=$(element),this.affixed=this.unpin=this.pinnedOffset=null,this.checkPosition()};Affix.VERSION="3.3.0",Affix.RESET="affix affix-top affix-bottom",Affix.DEFAULTS={offset:0,target:window},Affix.prototype.getState=function(scrollHeight,height,offsetTop,offsetBottom){var scrollTop=this.$target.scrollTop(),position=this.$element.offset(),targetHeight=this.$target.height();if(null!=offsetTop&&"top"==this.affixed)return offsetTop>scrollTop?"top":!1;if("bottom"==this.affixed)return null!=offsetTop?scrollTop+this.unpin<=position.top?!1:"bottom":scrollHeight-offsetBottom>=scrollTop+targetHeight?!1:"bottom";var initializing=null==this.affixed,colliderTop=initializing?scrollTop:position.top,colliderHeight=initializing?targetHeight:height;return null!=offsetTop&&offsetTop>=colliderTop?"top":null!=offsetBottom&&colliderTop+colliderHeight>=scrollHeight-offsetBottom?"bottom":!1},Affix.prototype.getPinnedOffset=function(){if(this.pinnedOffset)return this.pinnedOffset;this.$element.removeClass(Affix.RESET).addClass("affix");var scrollTop=this.$target.scrollTop(),position=this.$element.offset();return this.pinnedOffset=position.top-scrollTop},Affix.prototype.checkPositionWithEventLoop=function(){setTimeout($.proxy(this.checkPosition,this),1)},Affix.prototype.checkPosition=function(){if(this.$element.is(":visible")){var height=this.$element.height(),offset=this.options.offset,offsetTop=offset.top,offsetBottom=offset.bottom,scrollHeight=$("body").height();"object"!=typeof offset&&(offsetBottom=offsetTop=offset),"function"==typeof offsetTop&&(offsetTop=offset.top(this.$element)),"function"==typeof offsetBottom&&(offsetBottom=offset.bottom(this.$element));var affix=this.getState(scrollHeight,height,offsetTop,offsetBottom);if(this.affixed!=affix){null!=this.unpin&&this.$element.css("top","");var affixType="affix"+(affix?"-"+affix:""),e=$.Event(affixType+".bs.affix");if(this.$element.trigger(e),e.isDefaultPrevented())return;this.affixed=affix,this.unpin="bottom"==affix?this.getPinnedOffset():null,this.$element.removeClass(Affix.RESET).addClass(affixType).trigger(affixType.replace("affix","affixed")+".bs.affix")}"bottom"==affix&&this.$element.offset({top:scrollHeight-height-offsetBottom})}};var old=$.fn.affix;$.fn.affix=Plugin,$.fn.affix.Constructor=Affix,$.fn.affix.noConflict=function(){return $.fn.affix=old,this},$(window).on("load",function(){$('[data-spy="affix"]').each(function(){var $spy=$(this),data=$spy.data();data.offset=data.offset||{},null!=data.offsetBottom&&(data.offset.bottom=data.offsetBottom),null!=data.offsetTop&&(data.offset.top=data.offsetTop),Plugin.call($spy,data)})})}(jQuery),angular.module("ui.sortable",[]).value("uiSortableConfig",{}).directive("uiSortable",["uiSortableConfig","$timeout","$log",function(uiSortableConfig,$timeout,$log){return{require:"?ngModel",link:function(scope,element,attrs,ngModel){function combineCallbacks(first,second){return second&&"function"==typeof second?function(e,ui){first(e,ui),second(e,ui)}:first}var savedNodes,opts={},callbacks={receive:null,remove:null,start:null,stop:null,update:null};return angular.extend(opts,uiSortableConfig),angular.element.fn&&angular.element.fn.jquery?(ngModel?(scope.$watch(attrs.ngModel+".length",function(){$timeout(function(){element.data("ui-sortable")&&element.sortable("refresh")})}),callbacks.start=function(e,ui){ui.item.sortable={index:ui.item.index(),cancel:function(){ui.item.sortable._isCanceled=!0},isCanceled:function(){return ui.item.sortable._isCanceled},_isCanceled:!1}},callbacks.activate=function(){savedNodes=element.contents();var placeholder=element.sortable("option","placeholder");if(placeholder&&placeholder.element&&"function"==typeof placeholder.element){var phElement=placeholder.element();phElement=angular.element(phElement);var excludes=element.find('[class="'+phElement.attr("class")+'"]');savedNodes=savedNodes.not(excludes)}},callbacks.update=function(e,ui){ui.item.sortable.received||(ui.item.sortable.dropindex=ui.item.index(),ui.item.sortable.droptarget=ui.item.parent(),element.sortable("cancel")),"clone"===element.sortable("option","helper")&&(savedNodes=savedNodes.not(savedNodes.last())),savedNodes.appendTo(element),ui.item.sortable.received&&!ui.item.sortable.isCanceled()&&scope.$apply(function(){ngModel.$modelValue.splice(ui.item.sortable.dropindex,0,ui.item.sortable.moved)})},callbacks.stop=function(e,ui){!ui.item.sortable.received&&"dropindex"in ui.item.sortable&&!ui.item.sortable.isCanceled()?scope.$apply(function(){ngModel.$modelValue.splice(ui.item.sortable.dropindex,0,ngModel.$modelValue.splice(ui.item.sortable.index,1)[0])}):"dropindex"in ui.item.sortable&&!ui.item.sortable.isCanceled()||"clone"===element.sortable("option","helper")||savedNodes.appendTo(element)},callbacks.receive=function(e,ui){ui.item.sortable.received=!0},callbacks.remove=function(e,ui){ui.item.sortable.isCanceled()||scope.$apply(function(){ui.item.sortable.moved=ngModel.$modelValue.splice(ui.item.sortable.index,1)[0]})},scope.$watch(attrs.uiSortable,function(newVal){element.data("ui-sortable")&&angular.forEach(newVal,function(value,key){callbacks[key]&&("stop"===key&&(value=combineCallbacks(value,function(){scope.$apply()})),value=combineCallbacks(callbacks[key],value)),element.sortable("option",key,value)})},!0),angular.forEach(callbacks,function(value,key){opts[key]=combineCallbacks(value,opts[key])})):$log.info("ui.sortable: ngModel not provided!",element),void element.sortable(opts)):void $log.error("ui.sortable: jQuery should be included before AngularJS!")}}}]),angular.module("sampleApp",["ngRoute","appRoutes","MainCtrl","PresentCtrl","PresentService","ui.sortable","LoginCtrl","SignupCtrl","AdminCtrl","NavCtrl","UserService","ListCtrl"]),angular.module("appRoutes",[]).config(["$routeProvider","$locationProvider",function($routeProvider,$locationProvider){$routeProvider.when("/",{templateUrl:"views/home.html",controller:"MainController"}).when("/presents",{templateUrl:"views/presents.html",controller:"PresentController"}).when("/admin",{templateUrl:"views/admin.html",controller:"AdminController"}).when("/login",{templateUrl:"views/login.html",controller:"LoginController"}).when("/signup",{templateUrl:"views/signup.html",controller:"SignupController"}).when("/noUser",{templateUrl:"views/noUser.html"}).when("/list/:userId",{templateUrl:"views/list.html",controller:"ListController"}),$locationProvider.html5Mode(!0)}]),angular.module("AdminCtrl",[]).controller("AdminController",["$scope","$http",function($scope,$http){$scope.users=[],$scope.presents=[];var socket=io.connect("https://secret-dawn-8024.herokuapp.com");socket.on("news",function(data){console.log("omfg its working!"),console.log(data),socket.emit("my other event",{my:"data"})}),$scope.getUsers=function(){$http.get("/api/users").success(function(data){$scope.users=data,console.log(data)}).error(function(data){console.log("error: "+data)})},$scope.getPresents=function(){$http.get("/api/presents").success(function(data){$scope.presents=data,console.log(data)}).error(function(data){console.log("error: "+data)})},$scope.editPresent=function(){var tmpId=$scope.presents[1]._id;$scope.presents[1].title="edited title",console.log(tmpId),$http.put("/api/presents/"+tmpId,{title:$scope.presents[1].title,notes:$scope.presents[1].notes,link:$scope.presents[1].link}).success(function(data){console.log(data),console.log("hooray, present was edited!"),$scope.getPresents()}).error(function(data){console.log("error: "+data)})},$scope.deletePresent=function(){var tmpId=$scope.presents[0]._id;$http.delete("/api/presents/"+tmpId).success(function(data){console.log(data),$scope.getPresents()}).error(function(data){console.log("error: "+data)})},$scope.deleteUser=function(){var tmpId=$scope.users[0]._id;$http.delete("/api/users/"+tmpId).success(function(data){console.log(data),$scope.getUsers()}).error(function(data){console.log("error: "+data)})},$scope.addPresentsToUser=function(){var tmpId=$scope.users[1]._id,prId=$scope.presents[1]._id;$http.put("/api/users/"+tmpId,{pId:prId}).success(function(data){console.log(data),$scope.getUsers()}).error(function(data){console.log("error: "+data)})},$scope.getUserPresent1=function(){var tmpId=$scope.users[1]._id;$http.get("/api/users/"+tmpId).success(function(data){console.log(data)}).error(function(data){console.log("error: "+data)})},$scope.printUser1Present0=function(){console.log("User 1 Present 0"),console.log($scope.users[1].presents[0].title)}}]),angular.module("ListCtrl",[]).controller("ListController",["$scope","$rootScope","$http","$location",function($scope,$rootScope,$http,$location){function getIdFromURL(){var x=$location.path();return x=x.split("/list/"),x=x[1]}function presentCompare(a,b){return a.index<b.index?-1:a.index>b.index?1:0}$scope.presentUser=null,$scope.presents=[],$scope.sameUser=!1,$scope.hovering=null,$scope.getUserPresents=function(){var tmpId=getIdFromURL();$http.get("/api/users/"+tmpId).success(function(data){$scope.presentUser=data,$scope.presents=$scope.presentUser.presents,$scope.presents.sort(presentCompare),$scope.presentUser._id===$rootScope.activeUser._id&&($scope.sameUser=!0)}).error(function(data){console.log("error: "+data)})},$scope.test=function(){$scope.getUserPresents(),console.log("getting user presents for user "+getIdFromURL()),$scope.claim(2)},$scope.getActiveUser=function(){$http.get("/api/users/active").success(function(data){data.message?($rootScope.activeUser=null,$location.path("/login")):($rootScope.activeUser=data,$scope.getUserPresents())}).error(function(data){console.log("error: "+data)})},$scope.isClaimed=function(idx){return null===$scope.presents[idx].claimedBy?!1:!0},$scope.isClaimedByActiveUser=function(idx){return $scope.presents[idx].claimedBy===$rootScope.activeUser._id?!0:!1},$scope.isClaimedByOtherUser=function(idx){return $scope.isClaimed(idx)&&!$scope.isClaimedByActiveUser(idx)?!0:!1},$scope.claim=function(idx){$http.post("/api/presents/claim/"+$scope.presents[idx]._id+"/"+$rootScope.activeUser._id).success(function(){console.log("claimed present!"),$scope.getUserPresents()}).error(function(data){console.log("error: "+data)})},$scope.unClaim=function(idx){$http.post("/api/presents/unclaim/"+$scope.presents[idx]._id+"/"+$rootScope.activeUser._id).success(function(){console.log("unclaimed present"),$scope.getUserPresents()}).error(function(data){console.log("error: "+data)})},$scope.getActiveUser(),$scope.getUserPresents(),$scope.listURL="http://listmas.io"+$location.path()}]).directive("selectOnClick",function(){return function(scope,element){element.bind("click",function(){this.select()})}}),angular.module("LoginCtrl",[]).controller("LoginController",["$scope",function(){}]),angular.module("MainCtrl",[]).controller("MainController",["$scope","$http","$location",function($scope,$http,$location){$scope.signupUserAlreadyExists=!1,$scope.invalidUsernamePassword=!1,$scope.noUserFound=!1,$scope.submitForm=function(isValid){isValid&&alert("our form is dope")},$scope.loginSubmit=function(e,p){$http.post("/login",{email:e,password:p}).success(function(data){"logged in"===data.message?$location.path("/presents"):"wrong password"===data.message?(console.log("wrong password"),$scope.invalidUsernamePassword=!0,$scope.loginPassword=""):"no user found"===data.message?(console.log("no user found"),$scope.noUserFound=!0,$scope.loginPassword=""):($scope.loginPassword="",console.log("couldnt log in for some reason"))}).error(function(data){console.log("error: "+data)})},$scope.signupSubmit=function(n,e,p){$http.post("/signup",{email:e,password:p,nombre:n}).success(function(data){"signed up"===data.message?$location.path("/presents"):($scope.signupUserAlreadyExists=!0,$scope.signupName="",$scope.signupEmail="",$scope.signupPassword="",console.log("couldnt sign up"))}).error(function(data){console.log("error: "+data)})}}]),angular.module("NavCtrl",[]).controller("NavController",["$scope","$rootScope","$location","$http",function($scope,$rootScope,$location,$http){$rootScope.activeUser=null,$rootScope.loggedIn=!1;var socket=io.connect("https://secret-dawn-8024.herokuapp.com");socket.on("connection success",function(){console.log("beep boop connected to server"),socket.emit("client connected",{message:"connected up in angular dog!"}),$scope.getActiveUser()}),socket.on("user check",function(){socket.emit("client reply",{message:"got it bro"}),$scope.getActiveUser()}),$scope.isActive=function(route){return route===$location.path()},$scope.logOut=function(){$http.get("/logout").success(function(){console.log("logged out!"),$location.path(""),$scope.getActiveUser()}).error(function(data){console.log("error: "+data)})},$scope.getActiveUser=function(){$http.get("/api/users/active").success(function(data){$rootScope.activeUser=data.message?null:data}).error(function(data){console.log("error: "+data)})},$scope.activeUser=function(){return tmpString="not logged in",$rootScope.activeUser&&(tmpString=$rootScope.activeUser.name),tmpString},$scope.rootScopeUserExists=function(){return null!==$rootScope.activeUser?!0:!1}}]),angular.module("PresentCtrl",[]).controller("PresentController",["$scope","$rootScope","$http","$filter","$location","User",function($scope,$rootScope,$http,$filter,$location){function presentCompare(a,b){return a.index<b.index?-1:a.index>b.index?1:0}$scope.editing=null,$scope.presentUser=null,$scope.presents=[],$scope.tmpPresent={},$scope.getUserPresents=function(){$http.get("/api/users/active").success(function(data){$scope.presentUser=data,$scope.presents=$scope.presentUser.presents,$scope.presents.sort(presentCompare)}).error(function(data){console.log("error: "+data)})},$scope.getUserPresents(),$scope.updatePresentIndices=function(){for(var i=0;i<$scope.presents.length;i++)$scope.presents[i].index=i;console.log("reset present indices")},$scope.presentClientToDB=function(a){var asdf=$scope.presents[a]._id;$http.post("/api/presents/"+asdf,{title:$scope.presents[a].title,notes:$scope.presents[a].notes,link:$scope.presents[a].link,index:$scope.presents[a].index,claimedBy:null}).success(function(){}).error(function(data){console.log("error: "+data)})},$scope.totalPresents=function(){return null!==$scope.presents?$scope.presents.length:void 0},$scope.updateDBFromPresents=function(){for(var j=0;j<$scope.presents.length;j++)$scope.presentClientToDB(j)},$scope.addPresentFromForm=function(){$scope.presents.push({title:$scope.newPresentTitle,notes:$scope.newPresentNotes,link:$scope.newPresentLink,index:$scope.presents.length,claimedBy:null}),$http.post("/api/presents",{title:$scope.newPresentTitle,notes:$scope.newPresentNotes,link:$scope.newPresentLink,index:$scope.presents.length,claimedBy:null}).success(function(data){console.log("hooray, present added!"),console.log(data),$scope.hookUpPresentIdToUser(data._id),$scope.getUserPresents()}).error(function(data){console.log("error: "+data)}),$scope.newPresentTitle="",$scope.newPresentNotes="",$scope.newPresentLink=""},$scope.hookUpPresentIdToUser=function(prId){var tmpUserId=$scope.presentUser._id;$http.put("/api/users/"+tmpUserId,{pId:prId}).success(function(data){console.log(data)}).error(function(data){console.log("error: "+data)})},$scope.updatePresent=function(idx){$scope.presents[idx].title=$scope.tmpPresent.title,$scope.presents[idx].notes=$scope.tmpPresent.notes,$scope.presents[idx].link=$scope.tmpPresent.link,$scope.presents[idx].index=$scope.tmpPresent.index;var tmpId=$scope.presents[idx]._id;$http.post("/api/presents/"+tmpId,{title:$scope.presents[idx].title,notes:$scope.presents[idx].notes,link:$scope.presents[idx].link,index:$scope.presents[idx].index}).success(function(){$scope.getUserPresents()}).error(function(data){console.log("error: "+data)}),$scope.clearEditing()},$scope.openEditDialog=function(idx){$scope.editing=idx,$scope.tmpPresent.title=$scope.presents[idx].title,$scope.tmpPresent.notes=$scope.presents[idx].notes,$scope.tmpPresent.link=$scope.presents[idx].link,$scope.tmpPresent.index=$scope.presents[idx].index},$scope.clearEditing=function(){$scope.editing=null,$scope.tmpPresent={}},$scope.deletePresent=function(idx){var tmpId=$scope.presents[idx]._id;$http.delete("/api/presents/"+tmpId).success(function(){$scope.getUserPresents()}).error(function(data){console.log("error: "+data)}),$http.post("/api/users/"+$scope.presentUser._id+"/presents/"+tmpId).success(function(){$scope.getUserPresents()}).error(function(data){console.log("error: "+data)})},$scope.sortableOptions={stop:function(){console.log("stop"),$scope.updatePresentIndices(),$scope.updateDBFromPresents(),console.log($scope.presents)}},$scope.finalizeUser=function(){var tmpId=$scope.presentUser._id;$http.post("/api/users/finalize/"+tmpId).success(function(){$location.path("/list/"+tmpId)}).error(function(data){console.log("error: "+data)})},$scope.ifFinalizedThenSwitchToList=function(){console.log("if finalized then switch"),null!==$rootScope.activeUser&&(console.log("rootscope active user != null"),$rootScope.activeUser.finalized===!0&&(console.log("rootscope finalized = true"),$location.path("/list/"+$rootScope.activeUser._id)))},$scope.getActiveUser=function(){$http.get("/api/users/active").success(function(data){data.message?$rootScope.activeUser=null:($rootScope.activeUser=data,$scope.ifFinalizedThenSwitchToList())}).error(function(data){console.log("error: "+data)})},$scope.getActiveUser(),$scope.userLoaded=function(){return null===$scope.presentUser?!1:!0}}]),angular.module("SignupCtrl",[]).controller("SignupController",["$scope",function(){}]),angular.module("PresentService",[]).factory("Present",function(){function Present(title){this.title=title,this.notes=null,this.link=null}return Present}),angular.module("UserService",[]).service("User",["$http","$rootScope",function($http,$rootScope){}]);