...
angular.module('PresentCtrl', []).controller('PresentController', ['$scope', '$rootScope', '$http', '$filter', '$location', 'User', function($scope, $rootScope, $http, $filter, $location, User) {
  ...
  $scope.getUserPresents = function() {
    ...
    $http.get('/api/users/active')
      .success(function(data) {
        ...
        if ($scope.presentUser.brandNew === true) {
          $scope.needsModal = true;
        }

        console.log($scope.presents.length);
        $scope.presents.sort(presentCompare);

        // checks whether the user is new and updates accordingly
        $scope.userIsntNew();
      })
      .error(function(data) {
  ...
  $scope.presentClientToDB = function(a) {
    ...
    $http.post('/api/presents/' + asdf, {
        ...
        claimedBy: $scope.presents[a].claimedBy,
        published: $scope.presents[a].published
      })
      .success(function(data) {})
      .error(function(data) {
        console.log('error: ' + data);
  ...
  $scope.updatePresent = function(idx) {
    ...
    $http.post('/api/presents/' + tmpId, {
        ...
        claimedBy: $scope.presents[idx].claimedBy,
        published: $scope.presents[idx].published
      })
      .success(function(data) {
        $scope.getUserPresents();
      })
  ...
  $scope.publishPresent = function(idx) {
    $scope.presents[idx].published = true;
  ...
  $scope.publishAllPresents = function() {
    for (var k = 0; k < $scope.presents.length; k++)
      $scope.publishPresent(k);
  };
  ...
  $scope.helpEmail = 'santa@listmas.io';


  $scope.userIsFinalized = function() {
    if ($scope.presentUser) {
      if ($scope.presentUser.finalized === true)
        return true;
    } else
      return false;
  };

  $scope.userIsntNew = function() {
    if ($scope.presentUser !== null) {
      if ($scope.presentUser.brandNew === true) {
        $http.post('/api/users/notnew/' + $scope.presentUser._id)
          .success(function(data) {
            console.log('user aint new no more');
          })
          .error(function(data) {
            console.log('error: ' + data);
          });
